...
class ArchiveExtractor:
    ...
    def is_archive(self, file_path: Path) -> bool:
        ...
        if self.is_multipart_archive(file_path):
            ...
            if '.rar' in filename and not self.has_rarfile:
                self.logger.warning(f"Multi-part RAR file detected but rarfile library not available: {file_path.name}")
                return False
            # For .7z multi-part, check if py7zr is available
            if re.search(r'\.\d{3}$', filename) and not self.has_py7zr:
                self.logger.warning(f"Multi-part 7z file detected but py7zr library not available: {file_path.name}")
                return False
            return True
    ...
    async def has_video_files(self, archive_path: Path) -> bool:
        ...
        try:
            ...
            has_video = await asyncio.get_event_loop().run_in_executor(
                None, check_sync
            )
