...
class UserConfig:
    ...
    async def get_movies_path(self) -> Path:
        ...
        custom_path = await self.database.get_user_setting(self.user_id, "movies_path")
        if custom_path:
    ...
    async def get_tv_path(self) -> Path:
        ...
        custom_path = await self.database.get_user_setting(self.user_id, "tv_path")
        if custom_path:
            return Path(custom_path)
    ...
    async def get_tmdb_language(self) -> str:
        ...
        lang = await self.database.get_user_setting(self.user_id, "tmdb_language")
        if lang:
            return str(lang)
    ...
    async def get_subtitle_enabled(self) -> bool:
        ...
        enabled = await self.database.get_user_setting(self.user_id, "subtitle_enabled")
    ...
    async def get_subtitle_languages(self) -> List[str]:
        ...
        langs = await self.database.get_user_setting(self.user_id, "subtitle_languages")
        if langs:
    ...
    async def get_subtitle_format(self) -> str:
        ...
        fmt = await self.database.get_user_setting(self.user_id, "subtitle_format")
        if fmt:
            return str(fmt)
